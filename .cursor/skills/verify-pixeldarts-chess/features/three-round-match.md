...
A match is not one shoot-out. Players complete at least three full rounds (shoot, chase, six-ply chess) so persistent first-player order and the post-round-3 mechanic can actually appear. Stopping after round 1 is an incomplete pass.

## Sub-features

- `round-1` is a complete head-to-head: P1 sets, P2 chases, continuation animates.
- `round-2` starts from the new board and a new seed, with White still shooting first.
...
- Inspect the final chess position for as long as needed. The board must not
  advance on a timer. Press A to start round 2 (White throws first again).
...
- The helper scripts unequal scores through target hits and misses so sudden
  death does not take over this path.
- Fresh game at `title`.
...
- **After round 3.** Frame `r4_checkmate_unlocked.png` visibly says `CHECKMATE`
  / `UNLOCKED`. Its 1.5-second timer enters round 4; after round 4's dart result,
  `ContinuationRequest.allow_mate` is true.
- **Proof.** `summary.json` has `round_count >= 3`, `first_shooter: ["white","white","white"]`, three distinct `round_seed` values, chained FENs, `allow_mate=false` for rounds 1–3, and `round4_allow_mate=true`.
